...
struct BasicDomainReporter{T}
    description::String
end

struct Observation{T, U}
    value::T
    data::U
end
...
# Sample usage
reporter = BasicDomainReporter{TheTwoVectorsWithGreatestSineOfSums}("Sample Description")
observations = [Observation(TheVectorWithAverageClosestToPi(), [1.0, 2.0]),
                Observation(TheVectorWithAverageClosestToPi(), [2.0, 3.0]),
                Observation(TheVectorWithAverageClosestToPi(), [3.0, 4.0])]

create_report(reporter, 1, true, false, "sample_domain", observations)
