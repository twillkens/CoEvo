...
function laplacian_matrix(
    g::GNNGraph, T::DataType = eltype(g); dir::Symbol = :out, add_self_loops=true
)
...
function graphspectrum(g::GNNGraph; add_self_loops = true, dir = :both, usenorm::Bool = false)
    lp = usenorm ?
...
function padspec(spec1::Vector{Float64}, spec2::Vector{Float64})
...
function graph_distance(
    ...
    add_self_loops = true, dir = :both, p::Int = 2, dopad::Bool = true
)
    spec1 = graphspectrum(g1; add_self_loops = add_self_loops, dir = dir)
    spec2 = graphspectrum(g2; add_self_loops = add_self_loops, dir = dir)
    println(spec1)
    println(spec2)
    spec1, spec2 = dopad ? padspec(spec1, spec2) : clipspec(spec1, spec2)
    norm(spec1 - spec2, p)
end

function graph_distance(
    gs1::Vector{GNNGraph}, gs2::Vector{GNNGraph}; add_self_loops = true, dir = :both
)
