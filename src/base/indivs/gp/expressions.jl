export Expression, symbols, compile, height, nodes, length, depth, copyto!
export randsubexpr, getindex, setindex!
export isnum, iszeronum, isonenum, isdiv, isexpr, issym, isexprsym, isbinexpr
export evaluate, simplifyunary!, simplifybinary!, simplify!, infix, latex, show
export ExprNode, make_expr
"""
...
function Base.copyto!(dst::Expr, src::Expr)
    dst.head = src.head
...
function get_nfuncs(ex::Union{Expr, Symbol, Real})
    if isa(ex, Symbol) || isa(ex, Real)
        return 0
    else
        return 1 + sum(get_nfuncs(nex) for nex in ex.args[2:end])
    end
end

function Base.getindex(ex::Expr, idx::Int)
    if idx == 0
        ex
    else
...
function Base.setindex!(ex::Expr, subex, idx::Int)
    csize = cumsum(map(nodes, ex.args[2:end]))
    nidx = findfirst(i->i>=idx, csize)
    nex = ex.args[nidx+1]
...
function evaluate(ex::Expr, psyms::Dict{Symbol,Int}, vals::T...)::T where {T}
    exprm = ex.args
    exvals = (isexpr(nex) || issym(nex) ? 
        evaluate(nex, psyms, vals...) : 
        nex for nex in exprm[2:end])
    exprm[1](exvals...)
end
...
function evaluate(ex::Expr, psyms::Dict{Symbol,Function})
    exprm = ex.args
    exvals = (isexpr(nex) || issym(nex) ? evaluate(nex, psyms) : nex for nex in exprm[2:end])
    if issym(exprm[1])
        psyms[exprm[1]](exvals...)
    else
        exprm[1](exvals...)
    end
end

function evaluate(ex::Symbol, psyms::Dict{Symbol,Function})
    pidx = get(psyms, ex, 0)
    if pidx == 0
        @error "Undefined symbol: $ex"
    end
