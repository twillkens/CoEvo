...
function Species(
    spid::Symbol, phenocfg::PhenoConfig, initpop::Dict{IndivKey, I}
) where {I <: Individual}
    Species(spid, phenocfg, Dict{IndivKey, I}(), initpop)
end

function Species(
    spid::Symbol, phenocfg::PhenoConfig, initpop::Vector{I}
) where {I <: Individual}
    Species(spid, phenocfg, Dict(indiv.ikey => indiv for indiv in initpop))
end
