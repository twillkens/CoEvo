...
# 
# 
# function archive_new!(
#     gen::Int, c::CoevConfig, allsp::Dict{Symbol, <:Species},
# )
#     if gen % c.pfilter_t == 0
#         push!(c.spchache, gen => allsp)
#         lastsp = c.spchache[gen - c.pfilter_t]
#         modes_filtered = Set{IndivKey}()
#         for (spid, species) in lastsp
#             for (childkey, child) in species.children
#                 pid = first(child.pids)
#                 parent_tag = c.tags[IndivKey(spid, pid)]
#                 c.tags[childkey] = parent_tag
#             end 
#         end
#         jld2file = jldopen(c.jld2path, "a")
#         for (gen, allsp) in c.spchache
...
# end
# 
# function update_tags!(gen::Int, c::CoevConfig, nextsp::Dict{Symbol, <:Species})
#     if gen % c.pfilter_t == 0
#         for (spid, species) in nextsp
#             for (childkey, child) in species.children
#                 pid = first(child.pids)
#                 parent_tag = c.tags[IndivKey(spid, pid)]
#                 c.tags[childkey] = parent_tag
...
#         end
#     else
#         for species in values(nextsp)
#             for (tag, childkey) in enumerate(keys(species.children))
#                 c.tags[childkey] = tag
#             end 
#         end
#     end
