...
struct CompleteFunctionGraphPhenotypeState{P <: Phenotype} <: PhenotypeState
    phenotype::P
    node_values::Dict{Int, Float32}
end

function get_node_value(state::CompleteFunctionGraphPhenotypeState, node_id::Int)
    return state.node_values[node_id]
end

function get_phenotype_state(phenotype::CompleteFunctionGraphPhenotype)
    state = Dict(
        node.id => node.current_value
        for node in phenotype.nodes
    )
    state = CompleteFunctionGraphPhenotypeState(phenotype, state)
    return state
end
...
function get_node_median_value(
    #states::Vector{CompleteFunctionGraphPhenotypeState}, node_id::Int
