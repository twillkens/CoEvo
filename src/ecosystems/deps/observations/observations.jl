...
like [`OutcomeObservationConfiguration`](@ref).

# Key Types
- [`OutcomeObservation`](@ref): A structured type that captures the observed outcomes from interactions.
- [`OutcomeObservationConfiguration`](@ref): A configuration type that specifies how outcomes should be observed and tracked.
...
# Exports
The module exports: `OutcomeObservation`, `OutcomeObservationConfiguration`, and `get_outcomes`.
...
export OutcomeObservation, OutcomeObservationConfiguration, get_outcomes
