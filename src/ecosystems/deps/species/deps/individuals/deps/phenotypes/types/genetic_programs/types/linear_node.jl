...
using ....Genotypes.GeneticPrograms.Basic: Basic as GPGenotypeBasic
using .GPGenotypeBasic: BasicGeneticProgramGenotype
using .GPGenotypeBasic.Traverse: get_node
...
function linearize(genotype::BasicGeneticProgramGenotype, node_id::Int = genotype.root_id)
    ...
    function traverse(node_id)
        ...
        if isa(node.val, Function) && node.val == if_less_then_else
