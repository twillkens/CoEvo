...
using ....Individuals.Genotypes.GeneticPrograms.Basic: BasicGeneticProgramGenotype
using ...Abstract: PhenotypeCreator, Phenotype

import ...Interfaces: act, create_phenotype

include("linear_node.jl")
...
function read_tape!(pheno::BasicGeneticProgramPhenotype)
...
function set_tape!(pheno::BasicGeneticProgramPhenotype, data::Vector{<:Real})
    pheno.tape = tape
    pheno.head = length(data)
end

function act(pheno::BasicGeneticProgramPhenotype, tape::Vector{<:Real} = [0.0, π])
    try
        set_tape!(pheno, tape)
...
act(pheno::BasicGeneticProgramPhenotype, value::Real) = act(pheno, [value])
