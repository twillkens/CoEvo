...
function Base.show(io::IO, nodes::Vector{LinearNode})
    for (i, node) in enumerate(nodes)
        print(io, "$i: LinearNode(gid: $(node.gid), type: $(node.type), value: $(node.value), arity: $(node.arity), jump: $(node.jump_length))\n")
    end
end

function linearize(genotype::BasicGeneticProgramGenotype, node_gid::Int = genotype.root_gid)
    nodes = LinearNode[]
