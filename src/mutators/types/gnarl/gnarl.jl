...
function get_next_layer(genotype::GnarlNetworkGenotype, nodes::Vector{Float32})
    return [conn.destination for conn in genotype.connections if conn.origin in nodes && conn.destination != conn.origin]
end

function get_previous_layer(genotype::GnarlNetworkGenotype, nodes::Vector{Float32})
    return [conn.origin for conn in genotype.connections if conn.destination in nodes && conn.destination != conn.origin]
end

function exists_connection(genotype::GnarlNetworkGenotype, origin::Float32, destination::Float32)
    return any(conn -> conn.origin == origin && conn.destination == destination, genotype.connections)
end
...
function redirect_connection(
    ...
    valid_destinations = filter(dest -> is_valid_new_connection(genotype, node_to_remove_position, connection.origin, dest), available_destinations)
    
    if isempty(valid_destinations)
        return nothing
    end

    return rand(random_number_generator, valid_destinations)
end
