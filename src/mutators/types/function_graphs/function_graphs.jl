...
function add_function(
    ...
    input_connections = create_input_connections(random_number_generator, potential_input_node_ids, arity)
...
#                 node_id : rand(random_number_generator, [x.input_node_id for x in possible_redirects])
#             
    ...
    for (idx, connection) in enumerate(node.input_connections)
        if connection.input_node_id == node_to_delete_id
            ...
            if isempty(possible_redirects)
                ...
                new_input_node_id = rand(random_number_generator, [x.input_node_id for x in possible_redirects])
            end
...
function redirect_connection(
    ...
    input_connection_index = rand(random_number_generator, 1:length(genotype.nodes[node_id].input_connections))
...
function identity(
    random_number_generator::AbstractRNG, ::Counter, genotype::FunctionGraphGenotype, ::Dict{Symbol, GraphFunction}
)
    return deepcopy(genotype)
end
...
function inject_noise!(random_number_generator::AbstractRNG, genotype::FunctionGraphGenotype; std_dev::Float32=0.1f0)
    ...
    for (node_id, node) in genotype.nodes
        if !isempty(node.input_connections)  # Only for nodes with connections
            noise_values = randn(random_number_generator, length(node.input_connections)) .* std_dev  # Assuming normal distribution for noise
...
function mutate(
    ...
    for mutation in mutations
        genotype = mutation(random_number_generator, gene_id_counter, genotype, mutator.function_map)
    end
