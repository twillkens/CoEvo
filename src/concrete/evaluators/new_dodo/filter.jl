...
function get_filtered_indices(matrix::OutcomeMatrix, evaluator::NewDodoEvaluator)
    row_indices, col_indices = get_filtered_indices(
        matrix.data;
        filter_zero_rows = evaluator.filter_zero_rows, 
        filter_zero_columns = evaluator.filter_zero_columns, 
        filter_duplicate_rows = evaluator.filter_duplicate_rows,
        filter_duplicate_columns = evaluator.filter_duplicate_columns, 
    )
    return (row_indices, col_indices)
end

function get_filtered_matrix(matrix::OutcomeMatrix, evaluator::NewDodoEvaluator)
    #non_zero_row_indices = get_nonzero_row_indices(matrix.data)
    row_indices, column_indices = get_filtered_indices(matrix, evaluator)
