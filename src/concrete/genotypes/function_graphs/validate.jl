...
function has_cycle_nonrecurrent(genotype::FunctionGraphGenotype, start_node_id::Int)
    visited = Set{Int}()
...
function has_cycle_nonrecurrent(genotype::FunctionGraphGenotype)
    for node in genotype.nodes
...
function validate_genotype(genotype::FunctionGraphGenotype, last_mutation::String)
    # 1. Ensure Unique IDs
    ids = Set{Int}()
    for node in genotype.nodes
        id = node.id
