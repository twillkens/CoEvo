...
function get_bootstrapped_confidence_intervals(
    values::Vector{Float64}, confidence_level::Float64 = 0.95, n_samples::Int = 1000
)
    if length(values) == 0
        return [0.0, 0.0]
    end
    bootstrap_result = bootstrap(mean, values, BasicSampling(n_samples))
    _, lower_confidence, upper_confidence = first(bootstrap_confint(
        bootstrap_result, BasicConfInt(confidence_level)
    ))
    confidence_intervals = (lower_confidence, upper_confidence)
...
function get_confidence_intervals(
    ...
    std_error = std(values) / sqrt(length(values))
    z_score = quantile(Normal(0, 1), 1 - (1 - confidence_level) / 2)
    margin_of_error = z_score * std_error

    lower_confidence = mean_value - margin_of_error
    upper_confidence = mean_value + margin_of_error
